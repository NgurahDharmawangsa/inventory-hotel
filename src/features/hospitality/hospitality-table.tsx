...
import { Edit2, Trash2, Hotel, ArrowUpDown, ArrowUp, ArrowDown } from "lucide-react";

interface HospitalityTableProps {
  items: HospitalityWithRelations[];
  onEdit: (item: HospitalityWithRelations) => void;
  onDelete: (id: string) => void;
}

export function HospitalityTable({ items, onEdit, onDelete }: HospitalityTableProps) {
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center rounded-xl border border-dashed border-border bg-card">
        ...
        <h3 className="text-base font-bold text-foreground mb-1">No Hospitality Devices Found</h3>
        <p className="text-sm text-muted-foreground font-medium max-w-sm leading-relaxed">
          There are no hospitality devices registered in the inventory database matching your current query.
        </p>
      </div>
    );
  }
      ...
      <Table>
        <TableHeader className="bg-muted/30">
          <TableRow>
            ...
            {renderHeader("Room Number", "room_number")}
