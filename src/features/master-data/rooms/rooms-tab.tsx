...
export function RoomsTab() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        showNotification("error", res.error || "Failed to load rooms.");
      }
    } catch (err: any) {
  ...
  const handleFormSuccess = () => {
    setFormOpen(false);
    setSelectedItem(null);
    showNotification("success", `Room successfully ${selectedItem ? "updated" : "created"}.`);
    loadData();
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await deleteRoomAction(deleteId);
      if (res.success) {
        setDeleteOpen(false);
        setDeleteId(null);
        ...
        showNotification("error", res.error || "Failed to delete room.");
      }
    } catch (err: any) {
        ...
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-foreground">
              {selectedItem ? "Edit Room" : "Create New Room"}
            </DialogTitle>
            <DialogDescription className="text-sm font-medium text-muted-foreground">
              {selectedItem ? "Update room information." : "Add a new room to the inventory."}
            </DialogDescription>
          </DialogHeader>
          <div className="py-2">
            <RoomsForm
