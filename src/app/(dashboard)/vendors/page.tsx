...
export default function VendorsPage() {
  ...
  const loadVendors = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        showNotification("error", res.error || "Failed to load vendors list.");
      }
    } catch (err: any) {
      showNotification("error", err.message || "Failed to load vendors list.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFormSuccess = () => {
    setFormOpen(false);
    setSelectedItem(null);
    showNotification("success", `Vendor successfully ${selectedItem ? "updated" : "added"}.`);
    loadVendors();
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await deleteVendorAction(deleteId);
      if (res.success) {
        setDeleteOpen(false);
        setDeleteId(null);
        ...
        showNotification("error", res.error || "Failed to delete vendor record.");
      }
    } catch (err: any) {
      showNotification("error", err.message || "Failed to delete vendor record.");
    } finally {
  ...
  const stats = React.useMemo(() => {
    ...
    let withEmail = items.filter(item => item.email).length;
    let withPhone = items.filter(item => item.phone).length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl font-extrabold tracking-tight text-foreground">Vendor Directory</h1>
          <p className="text-sm font-medium text-muted-foreground">
            Manage corporate IT service partners, hardware warranty agents, and providers.
          </p>
        </div>
        <Button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded-xl border border-border bg-card p-4 flex items-center gap-4 shadow-2xs">
          ...
          <div className="space-y-0.5 leading-none">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Registered Vendors</span>
            <div className="text-xl font-extrabold text-foreground mt-1">
              {stats.total} <span className="text-xs font-semibold text-muted-foreground font-sans">partners</span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card p-4 flex items-center gap-4 shadow-2xs">
          ...
          <div className="space-y-0.5 leading-none">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">With Email</span>
            <div className="text-xl font-extrabold text-foreground mt-1">
              {stats.withEmail} <span className="text-xs font-semibold text-muted-foreground font-sans">suppliers</span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card p-4 flex items-center gap-4 shadow-2xs">
          ...
          <div className="space-y-0.5 leading-none">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">With Hotline</span>
            <div className="text-xl font-extrabold text-foreground mt-1">
              {stats.withPhone} <span className="text-xs font-semibold text-muted-foreground font-sans">agents</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Search vendors by name, contact person, or location details..."
          value={searchQuery}
        ...
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-foreground">
              {selectedItem ? "Edit Vendor Details" : "Register Vendor Partner"}
            </DialogTitle>
            <DialogDescription className="text-sm font-medium text-muted-foreground">
              Configure agency names, direct contact details, and registered service addresses.
            </DialogDescription>
          </DialogHeader>
          <div className="py-2">
            <VendorForm
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogTitle className="text-center text-lg font-bold text-foreground">
              Remove Vendor Partner?
            </DialogTitle>
            <DialogDescription className="text-center text-sm font-medium text-muted-foreground">
              Are you sure you want to permanently delete this vendor record? This will clear any associated active warranties.
            </DialogDescription>
          </DialogHeader>
